...
cpu_model=$(echo "$lscpu_out"  | egrep "name:" | awk '{print $3$4$5$6$7}' | xargs)
cpu_mhz=$(echo "$lscpu_out"  | egrep "MHz" | awk '{print $3}' | xargs)
l2_cache=$(echo "$lscpu_out"  | egrep "^L2" | awk '{print substr($3, 1, length($3)-1)}' | xargs)
